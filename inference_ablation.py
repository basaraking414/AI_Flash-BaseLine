...
# from sklearn.externals.array_api_compat.dask.array import linspace
...
def parse_args():
    ...
    parser.add_argument('--mask_dir', type=str, required=True,
                        help='Input directory or single mask path')
...
def build_model(config):
    """根据配置构建模型"""
    from basicsr.models.archs.my_restormer_arch import Restormer_AIFlash
    ...
    # 构建模型
    model = Restormer_AIFlash(**model_params)
...
def get_image_files(input_path):
    """获取输入图像文件列表"""
    extensions = ['jpg', 'jpeg', 'JPEG', 'png', 'PNG', 'bmp', 'BMP']
...
@torch.no_grad()
def process_image(model, img_path, gt_path,mask_path,output_dir, device, tile_size=None, tile_overlap=32):
    """处理单张图像"""
    ...
    mask_tensor = torch.from_numpy(mask).float().div(255.).permute(2, 0, 1).unsqueeze(0).to(device)

    # Pad 到 8 的倍数
    input_padded, padded_h, padded_w = pad_to_multiple(input_tensor, multiple=8)
    gt_padded, gt_h, gt_w = pad_to_multiple(gt_tensor, multiple=8)
    mask_padded, mask_padded_h, mask_padded_w = pad_to_multiple(mask_tensor, multiple=8)
...
def main():
    ...
    for iter_value in range(start_iter, end_iter + 1, step):
        # 构造权重文件路径
        weight_path = f"experiments/Restormer_AIFlash_v3_2/models/net_g_297000.pth"
        args.weights = weight_path

        output_dir_path = f"result/ablation/v3_2_297000"
        args.output_dir = output_dir_path

        gt_dir_path = f"datasets/train/gt"
        ...
        gt_files = get_image_files(gt_dir_path)
        ...
        # for img_path in tqdm(image_files, desc='Processing'):
            ...
            try:
                ...
                deltaE_all.append(deltaE)
            except Exception as e:
                print(f'\nError processing {img_path}: {e}')
                continue

        avg_psnr = sum(psnr_all) / len(psnr_all) if psnr_all else 0
        avg_deltaE = sum(deltaE_all) / len(deltaE_all) if deltaE_all else 0
        print(f'Average PSNR: {avg_psnr:.2f} dB | Average DeltaE: {avg_deltaE:.2f}')
