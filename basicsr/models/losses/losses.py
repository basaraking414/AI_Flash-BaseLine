...
class FaceLabLoss(nn.Module):
        ...
        def forward(self, pred, target, mask):
            """
            pred, target: [B,3,H,W], RGB in [0,1]
            mask: [B,1,H,W], face region = 1
            """

            # RGB -> LAB
            pred_lab = KC.rgb_to_lab(pred)
            tgt_lab = KC.rgb_to_lab(target)

            # pred_L, pred_a, pred_b = pred_lab[:, 0], pred_lab[:, 1], pred_lab[:, 2]
            # tgt_L, tgt_a, tgt_b = tgt_lab[:, 0], tgt_lab[:, 1], tgt_lab[:, 2]
            #
            # mask = mask.squeeze(1)

            # # -------- L channel (主导：曝光/闪光强度) --------
            # loss_L = torch.abs(pred_L - tgt_L)
            # loss_L = (loss_L * mask).sum() / (mask.sum() + self.eps)
            #
            # # -------- ab channel (辅助：肤色稳定) --------
            # loss_ab = (
            #         torch.abs(pred_a - tgt_a) +
            #         torch.abs(pred_b - tgt_b)
            # )
            # loss_ab = (loss_ab * mask).sum() / (mask.sum() + self.eps)
            #
            # loss = (self.lambda_l * loss_L + self.lambda_ab * loss_ab)
            ...
            if pixel_num == 0:
                ...
                diff_b = (pred_b - tgt_b)/128

                de = torch.sqrt(self.lambda_l*diff_L**2 +
                                self.lambda_ab * diff_a**2 +self.lambda_ab * diff_b**2 +
                                self.eps*self.eps)
                loss = (de * mask).sum() / (mask.sum() + self.eps)
                return self.loss_weight * loss


class CharbonnierLoss_mask(nn.Module):
    """Charbonnier Loss (L1)"""

    def __init__(self, loss_weight=1.0, reduction='mean', eps=1e-3, alpha=0):
        super(CharbonnierLoss_mask, self).__init__()
        self.loss_weight = loss_weight
        self.reduction = reduction
        self.eps = eps
        self.alpha = alpha

    def forward(self, x, y, mask,weight=None):
        diff = x - y
        loss = torch.sqrt(diff * diff + self.eps * self.eps)

        # mask: [B,1,H,W] → broadcast to 3 channels
        bg_mask = 1.0 - mask
        bg_mask = bg_mask.expand_as(loss)

        if weight is not None:
            loss = loss * weight

        loss = (loss * bg_mask).sum() / (bg_mask.sum() + self.eps)
                ...
                if layer_name in self.layer_weights:
                    if mask is not None:
                        ...
                        mask_resized = F.interpolate(
                            mask, size=x_features.shape[2:], mode='nearest'
                        )

                        diff = torch.abs(x_features - gt_features)
                        diff = diff * mask_resized

                        layer_loss = diff.sum() / (mask_resized.sum() + 1e-6)

                    else:
                        layer_loss = F.l1_loss(x_features, gt_features)
