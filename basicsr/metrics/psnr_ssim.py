...
def calculate_deltaE(img1, img2, mask, eps=1e-8):
    ...
    pred_lab = KC.rgb_to_lab(img1)
    tgt_lab = KC.rgb_to_lab(img2)
    ...
    de = torch.sqrt(diff_L ** 2 + diff_a ** 2 + diff_b ** 2 + eps * eps)
    loss = (de * mask).sum() / (mask.sum() + eps)
