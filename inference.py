...
@torch.no_grad()
def process_image(model, img_path, mask_path,output_dir, device, tile_size=None, tile_overlap=32):
    """处理单张图像"""
...
def main():
    ...
    for iter_value in range(start_iter, end_iter + 1, step):
        # 构造权重文件路径
        weight_path = f"experiments/Restormer_AIFlash_finetune_v7_freeze/models/net_g_{iter_value}.pth"
        args.weights = weight_path

        output_dir_path = f"result/test/my_aiflash_finetune_v7_freeze/fintune_v7_freeze_{iter_value}"
