...
class AIFlashModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):  # important，修改部分
        self.optimizer_g.zero_grad()
        # 添加混合精度训练
        if self.use_amp:
            with torch.amp.autocast(device_type='cuda'):
                preds = self.net_g(self.lq, self.mask)
        else:
            preds = self.net_g(self.lq, self.mask)
        #
        ...
        if self.cri_grad:
            l_grad = 0.
            for pred in preds:
                # bmask = self.boundary_mask(self.mask)
        ...
        # 开启混合精度训练后，反向传播和优化步骤需要使用 scaler 来处理
        if self.use_amp:
