...
| `configs/DC_LIDM.yaml` | `NAFNet_AIFlash_LIDM` | **当前不可用**：`basicsr` 中不存在该类；NAFNet+DC-LIDM 请用 `configs/nafnet_dclidm.yaml`（`NAFNet_LIDM`） |
...
1. ~~**【阻断】`basicsr/models/archs/RetinexMamba/` 子包缺失。**~~ **已解决**：论文未使用
   RetinexMamba / SwinIR（仅出现在 Related Work），对应的 `RetinexMamba_arch.py`、
   `SwinIR_arch.py` 及配置已从本项目中删除，`import basicsr.models.archs` 不再中断。
2. **`requirements.txt` 缺依赖**：`timm`（`Uformer_arch.py` 顶层导入，缺了会让整个 `archs` 导入失败）、
...
3. **`configs/DC_LIDM.yaml` 引用了不存在的网络类** `NAFNet_AIFlash_LIDM`，建模时会
   `ValueError: ... is not found`；NAFNet+DC-LIDM 请改用 `configs/nafnet_dclidm.yaml`。
4. **权重与配置覆盖不齐**：论文 Table I 中的 5 个 baseline 权重
   （KinD / Restormer / RetinexFormer / RetinexNet / ZeroDCE）目前没有对应配置，
   Table II 的 Baseline / V1 / V2 / V3 四个消融档也没有配置。可依据
   `experiments/<name>/train_*.log` 中的完整配置打印（`dict2str(opt)`）恢复。
...
8. **缺少论文级评测脚本**：源工程中的 `run_inference.py`（输出 PSNR/SSIM/ΔE、Params、FLOPs 的
   `report.json`，即 Table I/III/IV 的来源）与 `run_inference_alpha.py`（Fig. 12 的 α 扫描）
   未随本项目复制过来。

### 已验证 / 未验证

- ✅ 全部 Python 文件通过语法编译；包内相对/绝对导入除上述第 6 条外均可解析。
...
- ⚠️ **未做真实权重的前向/精度实跑**：当前环境未安装 `torch`。补装
  `torch/timm/tensorboard/torch-dwconv` 后建议先跑
