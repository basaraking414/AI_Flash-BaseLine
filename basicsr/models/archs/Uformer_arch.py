...
class Uformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        return torch.clamp(x + y,0,1) if self.dd_in ==3 else y
...
class Uformer_AIFlash(nn.Module):
    """Uformer 包装类，适配 BasicSR 接口"""

    def __init__(self, img_size=256, in_chans=3, dd_in=3,
                 embed_dim=32, depths=[1, 2, 8, 8, 2, 8, 8, 2, 1],
                 num_heads=[1,2,4,8,16,16,8,4,2],
                 win_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, token_projection='linear', token_mlp='leff',
                 shift_flag=True, modulator=False, cross_modulator=False, **kwargs):
        super().__init__()
        self.uformer = Uformer(
            ...
            token_mlp=token_mlp, shift_flag=shift_flag, modulator=modulator,
            cross_modulator=cross_modulator
        )

    def check_image_size(self, x):
        """Val/推理时将非正方形图像 resize 到正方形（最大边，对齐到128倍数）"""
        _, _, h, w = x.size()
        target_size = max(h, w)
        # 确保 target_size 能被 128 整除 (win_size=8 × downsample_factor=16)
        mod = 128
        target_size = ((target_size + mod - 1) // mod) * mod
    ...
    def forward(self, inp, mask=None, alpha=1.0):
        # 保存原始尺寸并 resize 到正方形
        inp, orig_h, orig_w = self.check_image_size(inp)
        if mask is not None:
            mask = F.interpolate(mask, size=(inp.shape[2], inp.shape[3]), mode='nearest')

        out = self.uformer(inp, mask=mask)
...
class Uformer_LIDM(nn.Module):
    """Uformer + DC-LIDM：带 Retinex 光照分解的 Uformer"""

    def __init__(self, num_heads=4, **kwargs):
        super().__init__()
        self.uformer = Uformer(**kwargs)
        from basicsr.models.archs.DC_LIDM import DC_LIDM
        bottleneck_ch = self.uformer.embed_dim * 16
    ...
    def forward(self, inp, mask=None, alpha=1.0):
        ...
        x_linear = srgb_to_linear(inp)

        # 手动拆分 Uformer 的 encoder/decoder
        y = self.uformer.input_proj(x_linear)
        y = self.uformer.pos_drop(y)

        conv0 = self.uformer.encoderlayer_0(y, mask=mask)
        pool0 = self.uformer.dowsample_0(conv0)
        conv1 = self.uformer.encoderlayer_1(pool0, mask=mask)
        pool1 = self.uformer.dowsample_1(conv1)
        conv2 = self.uformer.encoderlayer_2(pool1, mask=mask)
        pool2 = self.uformer.dowsample_2(conv2)
        conv3 = self.uformer.encoderlayer_3(pool2, mask=mask)
        pool3 = self.uformer.dowsample_3(conv3)

        bottleneck = self.uformer.conv(pool3, mask=mask)

        up0 = self.uformer.upsample_0(bottleneck)
        deconv0 = torch.cat([up0, conv3], -1)
        deconv0 = self.uformer.decoderlayer_0(deconv0, mask=mask)
        up1 = self.uformer.upsample_1(deconv0)
        deconv1 = torch.cat([up1, conv2], -1)
        deconv1 = self.uformer.decoderlayer_1(deconv1, mask=mask)
        up2 = self.uformer.upsample_2(deconv1)
        deconv2 = torch.cat([up2, conv1], -1)
        deconv2 = self.uformer.decoderlayer_2(deconv2, mask=mask)
        up3 = self.uformer.upsample_3(deconv2)
        deconv3 = torch.cat([up3, conv0], -1)
        deconv3 = self.uformer.decoderlayer_3(deconv3, mask=mask)

        decoder_out = self.uformer.output_proj(deconv3)

        # DC-LIDM
        B, C, H, W = inp.shape
        # bottleneck 是 (B, L, C) 格式，需要 reshape 成 (B, C, H', W')
        B_bt, L_bt, C_bt = bottleneck.shape
        H_bt = W_bt = int(L_bt ** 0.5)
        bottleneck_img = bottleneck.permute(0, 2, 1).reshape(B_bt, C_bt, H_bt, W_bt)
        result = self.dc_lidm(bottleneck_img, decoder_out[:, :, :H, :W],
        ...
        # resize 回原始尺寸
        if out_img.shape[2] != orig_h or out_img.shape[3] != orig_w:
            out_img = F.interpolate(out_img, size=(orig_h, orig_w), mode='bilinear', align_corners=False)

        self._intermediate = {
            'reflectance': F.interpolate(result['reflectance'], size=(orig_h, orig_w), mode='bilinear', align_corners=False),
            'env_light': F.interpolate(result['env_light'], size=(orig_h, orig_w), mode='bilinear', align_corners=False),
            'flash_map': F.interpolate(result['flash_map'], size=(orig_h, orig_w), mode='bilinear', align_corners=False),
            'alpha': result['alpha'],
            'illumination': F.interpolate(result['illumination'], size=(orig_h, orig_w), mode='bilinear', align_corners=False),
        }
        return out_img
